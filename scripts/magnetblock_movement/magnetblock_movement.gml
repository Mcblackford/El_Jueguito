...
if instance_exists(magnetObj) {
	...
	var _distance = 700;
	var _dist = point_distance(x+(sprite_width*0.5),y-(sprite_height*0.5),_target.x,_target.y);
	...
	if _dist <= _distance {
		if techo {
			...
			direction = point_direction (x, y-(sprite_height*0.5), x-(sprite_width*0.25), _target.y);
		} else if paredizq {
			direction = point_direction (x, y-(sprite_height*0.5), x+(sprite_width*0.25), _target.y);
		} else {
		...
		magnetobloquegrav = 0;
		if !collision_rectangle(x+(sprite_width*0.5)-10,y-(sprite_height*0.5)-10,x+(sprite_width*0.5)+10,y-(sprite_height*0.5)+10,magnetObj,true,true) || !place_meeting(x,y,colisionObj) {
			if suelo || techo {
				speed = 4 + (_dist*0.04);
				vspeed = 0;

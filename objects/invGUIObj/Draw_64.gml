if (!activo) && (alfasprite <= 0) {exit;}

draw_self();
draw_set_alpha(alfasprite);

if activo == true {
	if alfasprite < 1 {
		alfasprite += alfasumaspr;
	} else {
		alfasprite = 1;
	}
}

else if activo == false {
	if alfasprite > 0 {
		alfasprite -= alfasumaspr;
	} else {
		alfasprite = 0;
	}
}


var inventariosize=ds_list_size(inventarioObj.inventario);
var i1 =max( selector - 6, 0);
var i2 =min(i1 + 12, inventariosize);
var frame = (current_time / 300); //animacion momentanea para el selector

var dibujo = 0;

for (var i=i1;i<i2;i++){ 
	var item= inventarioObj.inventario[| i];
	var itemVar= global.items[item[INVENTARIO_ITEM]];
	if (itemVar[ITEM_VAR_TIPO]) == sec_act {
	draw_set_font(short_story_x16);
	if sec_act == TIPO_NORMI {
		draw_set_colour(#534027);
	} else {
		draw_set_colour(#283131);
	}
	var posy = dibujo * 39;
	
	if (dibujo == selector) {
		draw_sprite_ext(itemVar[ITEM_VAR_SPRITE], 0, x + 852, y + 204 , 5.5 , 5.5 ,0 ,c_white ,alfasprite);
		draw_text_ext(x + 680, y + 438, string(itemVar[ITEM_VAR_DESCRP]), 34 ,540)
		
		if sec_act == TIPO_NORMI {
			if botones == false {
				draw_sprite_ext(invselecSpr, frame, x + 156, y + 190 + posy, 2.88 , 3.12,0,c_white,alfasprite*0.75); // selector momentaneo
			} else {
				draw_sprite_ext(invselecSpr_1, frame, x + 156, y + 190 + posy, 2.88 , 3.12,0,c_white,alfasprite*0.5);
			}
		} else {
			if botones == false {
				draw_sprite_ext(invselecEspSpr, frame, x + 156, y + 190 + posy, 2.88 , 3.12,0,c_white,alfasprite); // selector momentaneo
			} else {
				draw_sprite_ext(invselecEspSpr_1, frame, x + 156, y + 190 + posy, 2.88 , 3.12,0,c_white,alfasprite*0.5);
			}
		}
	}
	
	draw_text(x + 175, y + 202 + posy,itemVar[ITEM_VAR_NAME]);

	if (itemVar[ITEM_VAR_TIPO] == TIPO_NORMI) {
		draw_set_halign(fa_right);
		draw_text(x + 610, y + 202 + posy, "x" + string(item[INVENTARIO_CANTIDAD]));
		draw_set_halign(fa_left);
	}

	dibujo++; // siempre ultimo
	}
}
	draw_set_font(scrambled_eggs_x30)
	// botones
	if (botones && sub_selector == 0) {
		draw_sprite_ext(invselecbut, frame, x + 672, y + 618, 2.845, 2.845,0, c_white, alfasprite); // Dibujar resaltado
	}
	draw_text(x + 690, y + 617, "TIRAR");

	//boton 2
	if (botones && sub_selector == 1) {
		draw_sprite_ext(invselecbut, frame, x + 962, y + 618, 2.845, 2.845,0, c_white, alfasprite);
	}
	draw_text(x + 1005, y + 617, "USAR");
	draw_set_font(short_story_x16)